...
from core.persistence.models import OrderDetail, Position
...
def place_order(market_price: float | int, order_obj: OrderDTOModel):
    ...
    try:
        ...
        _get_api_logger()(url=final_url, headers=headers, api_response=book_order, payload=order_obj, method="POST")
...
def update_sl_for(order_id: int, new_market_price: float):
    """
    This function is used to update the stop loss for a given order id and new market price
    inputs:
        order_id: int
    ...
    try:
        with orm_session() as session:
            ...
            db_position = session.exec(query).first()
            if db_position is None:
                fancy_print(
                    msg=f"Position id {order_id} not found",
                    border_color="red",
